...
This document provides example workflows demonstrating the Agent Orchestration System's capabilities.
...
```json
{
  ...
  "steps": [
    ...
    },
    {
      ...
      "agentId": "agent_claude_default",
      "input": {
        "prompt": "Suggest a musical mood for this poem (calm, energetic, mysterious): {{steps.poem.output.text}}"
      },
      "dependsOn": ["poem"]
    },
    {
...
const result = await orchestrateMapReduce(
  ...
  (item) => ({
    agentId: 'agent_claude_default',
    input: {
      prompt: `Summarize ${item} of the book in 2 sentences`
    }
  }),
...
Create reusable prompts with variable interpolation.
...
socket.emit('agent:workflow:execute', {
  workflow: {
    ...
    name: 'Real-time Demo',
    steps: [
      { id: 'step-1', name: 'Step 1', agentId: 'agent_claude_default', input: {} },
      { id: 'step-2', name: 'Step 2', agentId: 'agent_claude_default', input: {}, dependsOn: ['step-1'] }
    ]
  }
}, (response) => {
